...
namespace EmberCore.KernelServices.Command
{
    public class CommandService : KernelService, ICommandService
    {
        ...
        public CommandService(IOptionsSnapshot<CoreAppSetting> coreAppSetting, ILogger<CommandService> logger, ILifetimeScope scope)
        {
            Logger = logger;
        ...
        private CancellationTokenSource CreateOperationTimeLimitSource()
            => new CancellationTokenSource(TimeSpan.FromSeconds(CoreAppSetting.Value.CommandSourceOperationTimeLimit));
